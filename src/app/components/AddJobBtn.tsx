...
export default function AddJobBtn() {
  ...
  return (
    <DialogTrigger>
      ...
      <ModalOverlay className='fixed inset-0 z-20 flex items-center justify-center min-h-full overflow-y-auto bg-black/25'>
        <Modal className='w-full max-w-sm p-6 overflow-hidden text-left align-middle bg-white rounded shadow-xl'>
          <Dialog className='relative outline-none'>
            {({ close }) => (
              <form className='flex flex-col'>
                ...
                <TextField autoFocus className='mt-5'>
                  <Label>Job post URL</Label>
                  <Input
                    placeholder='https://...'
                    value={jobURL}
                    onChange={(e) => setJobURL(e.target.value)}
                    className='text-gray-700 appearance-none rounded block px-2.5 py-3 w-full ring-1 ring-gray-950/20 focus:ring-inset  focus:ring-2 focus:ring-blue-600 focus:outline-none hover:ring-gray-950/30 mt-2'
                  />
                </TextField>
                <Select className='flex flex-col mt-4'>
                  <Label>Job status</Label>
                  <Button className='flex items-center max-w-fit border border-gray-950/20 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-blue-600 py-3 px-2.5 text-gray-700 hover:bg-gray-100 hover:border-gray-950/30 rounded mt-2'>
